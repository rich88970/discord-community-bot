...
| 指令 | 功能 |
|---|---|
| `/claim` | 台北時間每小時 00／15／30／45 分進入新領取區間；預設領取 500,000 代幣 |
| `/daily` | 台北時間每天午夜刷新；預設領取 3,000,000 代幣 |
| `/balance`、`/leaderboard` | 查看餘額、戰績與排行榜 |
| `/give` | 玩家間轉帳 |
| `/bank info`、`deposit`、`withdraw` | 存款確認、提款鎖定與週期利息 |
| `/shop list`、`buy`、`inventory` | 購買及查看保險券、拆彈券、加倍券 |

預設新玩家餘額為 5,000；銀行每 4 小時計算一次 6% 遊戲利息，每次存款重新鎖定 24 小時。這些是遊戲平衡參數，不是金融產品。金額與利率集中在 `config.py`。`/claim` 的 15 分鐘分段目前寫在 `cogs/economy.py`，不能只改設定中的秒數就改變刷新規則。
...
部分遊戲整合道具與再玩一次操作。系統保存待結算下注，並定期檢查逾期待結算項目、退還資金與道具；不代表所有斷線或程式錯誤都能完整恢復當局畫面。
...
- `/invest buy`、`/invest sell`：使用遊戲代幣建立與平倉模擬部位，支援多空與槓桿設定。
...
公開整理版提供離線檢查，可驗證全部功能模組載入、指令註冊、設定檢查、說明訊息大小、圖片產生，以及臨時資料的寫入與恢復。執行：
