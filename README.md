...
| 指令 | 功能 |
|---|---|
| `/claim` | 台北時間每小時 00／15／30／45 分刷新；每次 50，不限每日領取次數 |
...
預設新玩家餘額為 **1,000**。銀行每 4 小時按全部存款發放 **1%** 遊戲利息，直接進錢包、不自動滾入本金，不設計息本金或利息金額上限，每期向下取整。每次存款重新鎖定 24 小時。

金額與利率集中在 `config.py`；RPG 副本獎勵位於 `rpg_data.py`。`/claim` 的 15 分鐘分段目前寫在 `cogs/economy.py`，不能只改設定中的秒數就改變刷新規則。
...
小遊戲最低下注 **10**，**不設下注金額或單局領回上限**。各遊戲的基礎回報率目標為 **99%**，已將百家樂和局退款、骰子圍骰、輪盤 0、拉霸各獎項與爬塔寶藏層機率納入計算。99% 是未計道具與整數取整的大量對局理論值，單局仍可能獲利或損失；實際代幣獎金向下取整。

保險券返還實際虧損的 50%；拆彈券免疫第一次炸彈，免疫本身不加獎金或倍率；「加倍券」沿用名稱，實際效果為正淨利 +25%。道具效果另計，因此使用道具的回報可能高於 99%。商店價格為 250／400／150，各種道具每日限買一次。
...
裝備每抽 **100**、技能每抽 **400**、首次職業免費／重抽 **1,000**。金幣副本依簡單／普通／困難／極難給 **600／1,200／2,000／3,000**，金幣與經驗副本共用 12 小時冷卻。一般 RPG 爬塔提供戰鬥金幣與經驗，Boss 可掉落素材。

### 虛擬投資

- `/invest price`、`/invest search`：台股、美股、BTC 代號與價格查詢。
- `/invest buy`、`/invest sell`：使用遊戲代幣建立與平倉模擬部位，支援多空與 **1–5 倍槓桿**；投入至少 100，不設單筆或總本金上限，仍需足夠錢包餘額。
- 採逐倉線性合約計算：預算包含開倉費，依部位數量與價差計算損益；開平倉各收名目價值 0.055%，以 0.5% 維持保證金率及預估平倉費判斷強制平倉。
- `/invest portfolio`、`/invest transactions`：查看費後資產估值、參考強平價、手續費及強平紀錄。背景定期檢查部位；已強平部位不能因行情反彈而恢復。
- 計算公式、資料來源與模擬範圍見 [虛擬投資使用說明](INVESTMENT.md)。這是代幣合約模擬，股票也使用相同模型；不代表各市場的真實融資規則。
...
investment.py          逐倉合約、Decimal 費用／損益計算與強平結算
...
公開整理版提供離線檢查，可驗證全部功能模組載入、指令註冊、設定檢查、說明訊息大小、圖片產生，以及臨時資料的寫入與恢復。另有經濟與投資測試，涵蓋領取冷卻、大額下注／獎金、無上限利息、99% 回報率計算、道具、交易費用、多空損益、加倉均價、逐倉強平、行情失敗，以及併發交易不重複入帳。執行：
